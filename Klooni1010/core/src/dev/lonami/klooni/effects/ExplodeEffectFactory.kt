...
class ExplodeEffectFactory : EffectFactory {
    ...
    override fun create(deadCell: Cell, culprit: Vector2): Effect = ExplodeEffect().apply {
    ...
    private class ExplodeEffect : Effect {
        ...
        override fun setInfo(deadCell: Cell, culprit: Vector2) {
            ...
            shards = arrayOfNulls(MathUtils.random(4, 6))
            for (i in shards.indices) shards[i] = Shard(deadCell.position, deadCell.cellSize)
        }

        override fun draw(batch: Batch) {
            isDone = true // assume we're death
            val translation = batch.transformMatrix.getTranslation(Vector3())
            var i = shards.size
            while (i-- != 0) {
                shards[i]!!.draw(batch, Gdx.graphics.deltaTime)
                isDone = isDone and (translation.y + shards[i]!!.pos.y + shards[i]!!.size < 0) // all must be dead
            }
        }
