...
class Piece {
    ...
    private constructor(cols: Int, rows: Int, rotateSizeBy: Int, colorIndex: Int) {
        ...
        shape = Array<BooleanArray?>(cellRows) { BooleanArray(cellCols) }
        for (i in 0..<cellRows) {
            for (j in 0..<cellCols) {
                shape[i]!![j] = true
            }
    ...
    private constructor(lSize: Int, rotateCount: Int, colorIndex: Int) {
        ...
        cellRows = lSize
        cellCols = cellRows
        shape = Array<BooleanArray?>(lSize) { BooleanArray(lSize) }
    ...
    fun draw(batch: SpriteBatch) {
        ...
        for (i in 0..<cellRows) for (j in 0..<cellCols) if (shape[i]!![j]) draw(c, batch, pos.x + j * cellSize, pos.y + i * cellSize, cellSize)
    }
