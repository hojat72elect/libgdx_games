...
class PieceHolder(
    ...
    val area: Rectangle
    private val pieces: Array<Piece?> = arrayOfNulls(count)
    ...
    private val originalPositions: Array<Rectangle?> = arrayOfNulls(count)
    ...
    private fun handFinished(): Boolean {
        for (i in 0..<count) if (pieces[i] != null) return false
    ...
    private fun takeMore() {
        for (i in 0..<count) pieces[i] = random()
    ...
    val availablePieces: com.badlogic.gdx.utils.Array<Piece?>
        get() {
            val result = com.badlogic.gdx.utils.Array<Piece?>(count)
            for (i in 0..<count) if (pieces[i] != null) result.add(pieces[i])
    ...
    fun dropPiece(): DropResult {
        ...
        if (heldPiece > -1) {
            val put: Boolean = enabled && board.putScreenPiece(pieces[heldPiece]!!)
            if (put) {
                if (soundsEnabled()) {
                ...
                if (soundsEnabled()) invalidPieceDropSound.play()
    ...
    fun update() {
        ...
        if (heldPiece > -1) {
            ...
            if (shouldSnapToGrid()) mouse.set(board.snapToGrid(piece, mouse))
    ...
    override fun read(input: DataInputStream) {
        ...
        val savedPieceCount = input.readInt()
        if (savedPieceCount != count) throw IOException("Invalid piece count saved.")

        for (i in 0..<count) pieces[i] = if (input.readBoolean()) Piece.read(input) else null
        updatePiecesStartLocation()
    }
    ...
    companion object {
        private const val DRAG_SPEED = 0.5f // Interpolation value ((pos -> new) / frame)
    }
}
