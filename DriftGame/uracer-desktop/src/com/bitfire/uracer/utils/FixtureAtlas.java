...
/**
 * Loads shapes (fixture sets) defined with the Box2D Editor and applies them to bodies. Has to be disposed to free some
 * resources.
 *
...
public final class FixtureAtlas {
    ...
    /**
     * Creates a new fixture atlas from the selected file. This file has to exist and to be valid.
     *
    ...
    public void createFixtures(Body body, String name, float width, float height) {
        createFixtures(body, name, width, height, null, new Vector2(0, 0), null);
    }
        ...
        public Vector2[][] getPolygons(float width, float height, Vector2 offset) {
            for (int i = 0; i < normalizedPolygons.length; i++) {
                for (int ii = 0; ii < normalizedPolygons[i].length; ii++) {
                    ...
                    this.polygons[i][ii].x += offset.x;
                    this.polygons[i][ii].y += offset.y;
                }
            }
            return polygons;
        }
