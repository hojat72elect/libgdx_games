...
/**
 * Core class of the Tween Engine. A Tween is basically an interpolation between two values of an object attribute. However, the
 * main interest of a Tween is that you can apply an easing formula on this interpolation, in order to smooth the transitions or
 * to achieve cool effects like springs or bounces.
 * <p/>
 * <p>
 * The Universal Tween Engine is called "universal" because it is able to apply interpolations on every attribute from every
 * possible object. Therefore, every object in your application can be animated with cool effects: it does not matter if your
 * application is a game, a desktop interface or even a console program! If it makes sense to animate something, then it can be
 * animated through this engine.
 * <p/>
 * <p>
 * This class contains many static factory methods to create and instantiate new interpolations easily. The common way to create a
 * Tween is by using one of these factories:
 * <p/>
 * <p>
 * - Tween.to(...)<br/>
 * - Tween.from(...)<br/>
 ...
 * <p>
 * The following example will move the target horizontal position from its current value to x=200 and y=300, during 500ms, but
 * only after a delay of 1000ms. The animation will also be repeated 2 times (the starting position is registered at the end of
 * the delay, so the animation will automatically restart from this registered position).
 * <p/>
 *
 * <pre>
 * {@code
 ...
 * </pre>
 * <p>
 * Tween life-cycles can be automatically managed for you, thanks to the {@link TweenManager} class. If you choose to manage your
 * tween when you start it, then you don't need to care about it anymore. <b>Tweens are <i>fire-and-forget</i>: don't think about
...
public final class Tween extends BaseTween<Tween> {
     ...
     * implementing the registered class, or inheriting from it.
     *
    ...
    public static Tween to(Object target, int tweenType, float duration) {
        ...
        tween.ease(Quad.Companion.getINOUT());
    ...
    /**
     * Sets the target value of the interpolation. The interpolation will run from the <b>value at start time (after the delay, if
     * any)</b> to this target value.
     * <p/>
     * <p>
     * To sum-up:<br/>
    ...
    /**
     * Sets the target values of the interpolation. The interpolation will run from the <b>values at start time (after the delay,
     * if any)</b> to these target values.
     * <p/>
     * <p>
     * To sum-up:<br/>
