 ...
 */
class AnimationSprite(private val frameDuration: Float, keyFrames: GdxArray<out Sprite>) {

    val animationDuration = keyFrames.size * frameDuration
    private val spriteFrames = arrayOfNulls<Sprite>(keyFrames.size)
    ...
    fun getKeyFrameIndex(stateTime: Float): Int {
        ...
        when (playMode) {
            ...
            PlayMode.LOOP_PINGPONG -> {
                ...
                if (frameNumber >= spriteFrames.size) frameNumber = (spriteFrames.size - 2 - (frameNumber - spriteFrames.size))
            }
