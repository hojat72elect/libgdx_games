...
class ParallaxBackground(
    ...
    fun render(delta: Float) {
        ...
        for ((region, parallaxRatio, startPosition, padding, width, height) in layers) {
            var currentX = -camera.position.x * parallaxRatio.x % (region.regionWidth + padding.x)

            if (speed.x < 0) currentX -= (region.regionWidth + padding.x)

            do {
                var currentY =
                    -camera.position.y * parallaxRatio.y % (region.regionHeight + padding.y)
